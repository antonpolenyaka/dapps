...
contract notas {
    ...
    // Mapping asignatura a las revisiones de los alumnos de pidan revisiones de examen de esta asignatura
    ...
    function Evaluar(string memory _asignatura, string memory _idAlumno, uint _nota) public UnicamenteProfesor(msg.sender){
        ...
        bytes32 hash_AsignaturaIdAlumno = keccak256(abi.encodePacked(_asignatura,_idAlumno));
        // Relacion entre el hash de la identificacion del alumno y su nota
        Notas[hash_AsignaturaIdAlumno] = _nota;
        // Emision del evento
        emit alumno_evaluado(hash_AsignaturaIdAlumno);
    }
    ...
    function VerNotas(string memory _asignatura, string memory _idAlumno) public view returns(uint) {
        ...
        uint nota_alumno = Notas[hash_AsignaturaIdAlumno];
        // Visualizar la nota 
        return nota_alumno;
    } 
